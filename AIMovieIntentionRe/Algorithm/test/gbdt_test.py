...
def sim_eval(text, label_text):
        ...
        print('导入已训练相似性相关模型......')
        dictionary = corpora.Dictionary.load('../resource/model/similarity/dict.txt')
...
def evaluate_gbdt(text):
    print('gbdt模型的意图识别......')
    gbdt = joblib.load("../resource/model/gbdt.m")
    vocab_path = '../resource/model/vocab'
    ...
    return y_label[0], y_pred[0], va

def filter_predict(sim_pred, sim_score, gbdt_pred, gbdt_score):
    if float(sim_score) < 0.53:  # 过滤掉与语料不相关的句子/意图
        pred = 'null'
    elif float(sim_score) > 0.75:
        pred = sim_pred
...
if __name__ == "__main__":
    # 导入字典和语料标签
    label = joblib.load("../resource/data/corpus_label.data")
    ...
    print(sim_estimate, similarity_score, y_label, y_pred[y_label], va)
